...
export default function MitraSidebar({ 
  ...
  const menuItems = [
    ...
    { name: "Buat Project Baru", href: "/dashboard/mitra/projects/create", icon: PlusCircle },
    { name: "Eksplor Project", href: "/projects", icon: Compass },
    { name: "Profil Saya", href: "/onboarding/mitra", icon: User },
  ];
    ...
    <div className="flex flex-col h-full bg-slate-950 text-slate-100 p-6 justify-between select-none">
      <div className="space-y-8">
        ...
        <div className="space-y-2">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-3 mb-3">Partner Menu</p>
          {menuItems.map((item) => {
      ...
      <div className="space-y-4 pt-4 border-t border-slate-900">
        <div className="flex items-center gap-3 px-2">
          <div className="w-8 h-8 rounded-lg bg-emerald-600 text-white flex items-center justify-center font-bold text-xs uppercase shadow-sm">
            {mitraName[0]}
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-xs font-extrabold text-white truncate">{mitraName}</p>
